        ...
        <div class="row">
            <div class="mb-3">
                ...
                <select name="grade_level" id="grade_level" class="form-select @error('grade_level') is-invalid @enderror">
                    ...
                    <option value="10" {{ old('grade_level', $classroom->grade_level) == '10' ? 'selected' : '' }}>10</option>
                    <option value="11" {{ old('grade_level', $classroom->grade_level) == '11' ? 'selected' : '' }}>11</option>
                    <option value="12" {{ old('grade_level', $classroom->grade_level) == '12' ? 'selected' : '' }}>12</option>
                </select>
                @error('grade_level')
            ...
            <div class="mb-3">
                ...
                <input type="text" name="major" id="major" class="form-control @error('major') is-invalid @enderror"
                    value="{{ old('major', $classroom->major) }}" placeholder="Masukkan jurusan atau program">
                @error('major')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="mb-3">
                ...
                <input type="number" class="form-control @error('class_number') is-invalid @enderror" name="class_number"
                    id="class_number" placeholder="1" value="{{ old('class_number', $classroom->class_number) }}" min="1">
                @error('class_number')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="mb-3">
                ...
                <input type="number" class="form-control @error('capacity') is-invalid @enderror" name="capacity"
                    id="capacity" value="{{ old('capacity', $classroom->capacity ?? 36) }}" min="1">
                @error('capacity')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="ms-3 mb-3">
                <div class="form-check">
                    <input class="form-check-input @error('is_active') is-invalid @enderror" type="checkbox" id="is_active"
                        name="is_active" value="1" @checked(old('is_active', $classroom->is_active))
